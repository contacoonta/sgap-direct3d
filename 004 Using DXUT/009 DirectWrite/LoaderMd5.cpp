...
Mesh* LoaderMd5::BuildMeshFromFile(LPCWSTR wfilename)
{
	...
	MeshSkeletal* pmd5 = static_cast<MeshSkeletal*>(pmesh);
...
BOOL LoaderMd5::BuildAnimationFromFile(LPCWSTR wfilename, Mesh* mesh)
{
	BOOL br = TRUE;
	...
	MeshSkeletal* pmd5 = static_cast<MeshSkeletal*>(mesh);

	br = LoadMD5Anim(wfilename, pmd5->m_model);
	if (br == FALSE)
		MessageBox(0, wfilename, L"Error", MB_OK);
						...
						std::vector<std::wstring> texFileNameArray )
{
	...
	std::wifstream fileIn(strpathW);				// Open file
	...
	if (fileIn)										// Check if the file was opened
	{
		while (fileIn)								// Loop until the end of the file is reached
		{
			...
			else if (checkString == L"mesh")
			{
				...
				while (checkString != L"}")			// Read until '}'
				{
					...
					if (checkString == L"shader")		// Load the texture or material
					{
						...
						if (!alreadyLoaded)
						{
							...
							wstring wpath = L"textures\\";
							wpath += fileNamePath;
							DXUTFindDXSDKMediaFileCch(strpathW, sizeof(strpathW) / sizeof(WCHAR), wpath.c_str());
							hr = DXUTCreateShaderResourceViewFromFile(DXUTGetD3D11Device(), strpathW, &tempMeshSRV);
							if (SUCCEEDED(hr))
							{
				...
				{
					VERTEXSKELETAL tempVert = subset.vertices[i];
				...
				DXUTGetD3D11Device()->CreateBuffer(&indexBufferDesc, &iinitData, &subset.indexBuff);
				...
				vertexBufferDesc.ByteWidth = sizeof(VERTEXSKELETAL)* subset.vertices.size();
				...
				HRESULT hr = DXUTGetD3D11Device()->CreateBuffer(&vertexBufferDesc, &vertexBufferData, &subset.vertBuff);
				if (FAILED(hr))
					return FALSE;
...
/*
*/
bool LoaderMd5::LoadMD5Anim(std::wstring filename, Model3D& MD5Model)
{
	ModelAnimation tempAnim;						// Temp animation to later store in our model's animation array

	WCHAR strpathW[256] = {};
	DXUTFindDXSDKMediaFileCch(strpathW, sizeof(strpathW) / sizeof(WCHAR), filename.c_str());

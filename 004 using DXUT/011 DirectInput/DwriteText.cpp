...
{
	// DirectX::Color 를 DWrite 용 Color 로 바꾸기
...
HRESULT DwriteText::Initialize()
{
	...
	hr = m_factory->CreateFontWrapper(DXUTGetD3D11Device(), L"맑은고딕", &m_fontwrap);
...
{
	// 최대 문자열 수를 초과하면 , 앞에것을 지운다.
	if (m_maxline <= m_strings.size())
	{
		m_strings.pop_front();
	}

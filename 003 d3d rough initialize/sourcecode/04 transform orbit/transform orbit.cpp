/*
	...
	과제 ] 태양계 만들기
		...
		- 확장] 각각 색상을 다르게 설정-> CONSTANT BUFFER
*/
...
ID3D11RenderTargetView* g_rtview = NULL;	// backbuffer 
...
ID3D11DepthStencilView* g_depthStencilView = NULL; // 깊이버퍼
	...
	while (WM_QUIT != msg.message)
	{
		...
		else //MainLoop
		{
			...
			*/
			...
			rot += 0.0001f;			
			g_ParentMat = XMMatrixRotationRollPitchYaw(rot*2.0f, 0.0f, 0.0f);
			g_ParentMat = g_ParentMat * XMMatrixTranslation(0.0f, 5.0f, 0.0f);


			XMMATRIX mscale = XMMatrixScaling(0.3f, 0.3f, 0.3f);
			XMMATRIX mrot = XMMatrixRotationRollPitchYaw(10.0f*rot, 0.0f, 0.0f);
			XMMATRIX mtrans = XMMatrixTranslation(0.0f, 5.0f, 0.0f);
			g_ChildMat = (mscale * mrot * mtrans) * g_ParentMat;

			// 백버퍼 지우기
			float ClearColor[4] = { 0.0f, 0.125f, 0.3f, 1.0f };
			g_devcon->ClearRenderTargetView(g_rtview, ClearColor);
			// 뎁스스텐실 지우기
			g_devcon->ClearDepthStencilView(g_depthStencilView,
...
HRESULT		InitDevice()
{
	...
	*/
	...
	hr = g_device->CreateTexture2D(&depthDesc, NULL, &g_depthStencil);
	if (FAILED(hr))
		return hr;
	...
	hr = g_device->CreateDepthStencilView(g_depthStencil, &DSVDesc,
													&g_depthStencilView);
	if (FAILED(hr))
		return hr;

	// Devcon 에 렌더타겟 뷰 랑 뎁스스텐실 뷰 를 연결
	g_devcon->OMSetRenderTargets(1, &g_rtview, g_depthStencilView);
...
void RenderTriangle()
{
	...
	*/
	...
	cb.outColor = XMFLOAT4(1.0f, 0.0f, 0.0f, 1.0f);
	...
	/*
		두번째 박스
	*/
	cb.mworld = XMMatrixTranspose(g_ChildMat);
	cb.outColor = XMFLOAT4(0.0f, 1.0f, 1.0f, 1.0f);
	// 셰이더와 소스를 연결
	g_devcon->UpdateSubresource(g_constantBuffer, 0, NULL, &cb, 0, 0);
	g_devcon->DrawIndexed(36, 0, 0);

                                      ...
                                      void* pUserContext )
{
	...
	LoaderMd5 loadermd5;
	g_mesh[0] = loadermd5.BuildMeshFromFile(L"models\\boy.md5mesh");

	LoaderObj loaderobj;
	g_mesh[1] = loaderobj.BuildMeshFromFile(L"models\\teapot.obj");
                                  ...
                                  double fTime, float fElapsedTime, void* pUserContext )
{
	...
	XMFLOAT4 LitDir = XMFLOAT4(0.577f, 0.85f, -0.6f, 1.0f);
	...
	*/
	...
	mworld = XMMatrixTranslation(-40.0f, .0f, .0f);
	XMStoreFloat4x4(&(cb.world), XMMatrixTranspose(mworld));
	g_shader->RenderPrepare(&cb);
	g_mesh[0]->Render(g_shader);

	mworld = XMMatrixTranslation(40.0f, .0f, .0f);
	XMStoreFloat4x4(&(cb.world), XMMatrixTranspose(mworld));
	g_shader->RenderPrepare(&cb);
	g_mesh[1]->Render(g_shader);
}
...
{
	SAFE_DELETE(g_mesh[0]);
	SAFE_DELETE(g_mesh[1]);
...
int WINAPI wWinMain( _In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow )
{
    ...
    DXUTCreateWindow( L"002 LoaderMd5" );

...
HRESULT Mesh::BuildFromObj(LPCWSTR wfilename)
{
	...
	hr = ObjLoader::CreateModelFromFile(&m_objloader, strpathW, *this);
	if (FAILED(hr))
		return hr;
...
*/
HRESULT Mesh::BuildFromFbx(LPCWSTR wfilename)
{
	HRESULT hr = S_OK;
	...
	hr = FbxLoader::CreateModelFromFile(&m_fbxloader, strpathW, *this);
	if (FAILED(hr))
		return hr;
